...
4. Забери файли з `out/`:
   ...
   - `changelog-<дата>.csv` — день-до-дня диф (те саме, що друкується в консоль
     як "День-до-дня"), але збережене у файл: по рядку на кожну подію
     (`added`/`removed`/`changed`) з qty/price-дельтами і прапорцем зміни
     специфікації. З'являється лише коли є попередній знімок для порівняння.
